...
All code lives under `src/`. Commands below assume you're running from
this repo's root (`PlaygroundCpp/`).

## Building & running

Compile and run any file with:
```
g++ -std=c++17 -O2 src/questions/1_max_gain_streak.cpp -o run && ./run
```
...
on, then:
```
watchexec -w src/questions -e cpp -- ./run.sh
```
